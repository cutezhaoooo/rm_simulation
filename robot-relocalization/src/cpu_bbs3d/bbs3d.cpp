...
BBS3D::BBS3D()
: v_rate_(2.0),
  num_threads_(std::max(1, static_cast<int>(std::thread::hardware_concurrency()))),  // Auto-detect CPU cores
  ...
  std::cout << "[CPU BBS3D] Using " << num_threads_ << " threads for parallel computation" << std::endl;
}
...
void BBS3D::set_tar_points(const std::vector<Eigen::Vector3d>& points, double min_level_res, int max_level) {
  ...
  void BBS3D::set_tar_points(const std::vector<Eigen::Vector3f>& points, double min_level_res, int max_level) {
    // 将 float 点云转换为 double 类型
    std::vector<Eigen::Vector3d> points_double;
    points_double.reserve(points.size());

    std::transform(
        points.begin(), points.end(),
        std::back_inserter(points_double),
        [](const Eigen::Vector3f& p) { return p.cast<double>(); }
    );

    // 调用原始函数处理转换后的点云
    set_tar_points(points_double, min_level_res, max_level);
}
...
void BBS3D::set_trans_search_range(const Eigen::Vector3d& min_xyz, const Eigen::Vector3d& max_xyz) {
  ...
  // 调用原始函数处理转换后的点云
  set_trans_search_range(points_double);
}
